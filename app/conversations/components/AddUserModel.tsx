...
export function AddUserModel(){
    ...
    const createConversation = useCallback((user : User)=>{
        setIsLoading(true);
    ...
    const searchUser : SubmitHandler<FieldValues> = (data)=>{
        setIsLoading(true)
        axios.post("../../api/searchUser", data)
        .then((response)=>{
            const {data} =  response
            setUser(data);
            console.log(data)
  ...
  return (
    ...
    <ConfirmationDialog modifyTrigger={false} title='Add New User' open={open} setOpen={setOpen}>
        ...
        <form onSubmit={handleSubmit(searchUser)}>
            <Input type='text' required={true} disabled={isLoading} register={register} id="email_id" placeholder='Type user email or id' />
            <LoadingButton className='mt-2' type='submit' isLoading={isLoading} disabled={isLoading}>
                Search
            </LoadingButton>
        </form>
        ...
        <DialogFooter className='mt-6'>
            <div className='flex gap-2 w-full justify-end'>
                <LoadingButton disabled={isLoading} variant="outline" onClick={()=>{ 
                    setUser(null);
                    setValue("email_id", "");
                    setOpen(false);
                }}>
