...
export const MessageBox: React.FC<MessageBoxProps> = ({ isLast, message }) => {
  ...
  const messageContainer = clsx(
    "rounded-3xl flex max-w-full w-fit relative text-base font-normal text-black",
    !isOwn
      ? "bg-white justify-end rounded-tl-none"
      : "bg-pink-500 justify-start rounded-tr-none",
    message.image ? "py-0 px-0 overflow-hidden transition" : "py-2 px-3 sm:px-4"
  );
