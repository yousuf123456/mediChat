  ...
  return (
    <ConfirmationDialog title={title} modifyTrigger={modifyTrigger} open={open} setOpen={setOpen}>
        <form onSubmit={handleSubmit(onSubmit)}>
            <Input id="name" disabled={isLoading} register={register} placeholder='Type New Name' required={false} type='text' />
            <div className='mt-6 w-full flex gap-2 justify-end'>
                <LoadingButton variant={"outline"} disabled={isLoading} onClick={()=>setOpen(false)}>
                    Cancel
                </LoadingButton>
                <LoadingButton disabled={isLoading} isLoading={isLoading} type='submit' className='bg-blue-600 rounded-sm hover:bg-blue-800'>
                    Save
                </LoadingButton>
            </div>
        </form>
